$(function () {
    const pclist = ("00-1, 00-2, 00-3, 00-4, 00-5, 00-6, 00-7, 00-8," +
        ...
        "99-1, 99-2, 99-3, 99-4, 99-5, 99-6, 99-7, 99-8").split(',');

    function makeId(i) {
        return 'boxPc' + ("000" + i.toString(16).toUpperCase()).substr(-3);
    }
    function getBank() {
        return _items[$('#gbPcMap').attr('id')].val;
    }
    function bankId(i) {
        return 'ID_SYSTEM_PC_MAP_BANK' + i + '_PC';
    }
    function updateValue(bId, id, i) {
        $('#' + id).trigger('elf-update', $es.getParamValue(bId, i));
    }
    function pcMap() { };
    pcMap.prototype = {
        make: function () {
            $es.dialog.openAutoClose('#dialog_working', 300, function () {
                var bId = bankId(getBank());
                for (var i = 0; i < 128; i++) {
                    var id = makeId(i);
                    modifySelBox(id, pclist, 0);
                    updateValue(bId, id, i);
                }
            });
        },
        unmake: function () {
            for (var i = 0; i < 128; i++) {
                modifySelBox(makeId(i), [""], 0);
            }
        }
    }
    ...
    $('#gbPcMap').on('elf-changed', function (e, v) {
        $es.dialog.openAutoClose('#dialog_working', 300, function () {
            var bId = bankId(getBank());
            for (var i = 0; i < 128; i++) {
                updateValue(bId, makeId(i), i);
            }
        });
    });
    $("[id^='boxPc']").on('elf-changed', function (e, v) {
        var index = parseInt(this.id.match(/boxPc([A-F0-9]+)/)[1], 16);
        $es.setParamValue(bankId(getBank()), index, v);
    });
});
